    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      */}
      <button
        ...
        aria-hidden="true"
        tabIndex={-1}
        // A plain black scrim rather than a token: the veil has to stay dark
        // over both a light and a dark page, which no single token does.
        className="absolute inset-0 h-full w-full cursor-default bg-black/40 dark:bg-black/70"
        onClick={onClose}
      />
      <div
        ref={panelRef}
        ...
        tabIndex={-1}
        className={cn(
          'relative z-10 flex max-h-[90svh] w-full flex-col rounded-t-3xl bg-surface-raised shadow-xl outline-none',
      ...
      >
        <header className="flex items-start justify-between gap-4 border-b border-border p-4">
          ...
          <button
            ...
            className="-m-1 inline-flex size-11 items-center justify-center rounded-xl text-muted hover:bg-surface-sunken hover:text-primary"
          >
            <X className="size-5" />
          </button>
        </header>
